...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            # Past-cycle extract, generated by build_static.py. Served from
            # whichever public dir exists so the page has one code path.
            if path == "/past_cycles.json":
                for d in ("public", "public_demo"):
                    fp = os.path.join(HERE, d, "past_cycles.json")
                    if os.path.exists(fp):
                        with open(fp, "rb") as f:
                            return self._send(200, f.read())
                return self._send(404, {"error": "no past_cycles.json; run build_static.py"})
